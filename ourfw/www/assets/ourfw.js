(function(){
  const q=(s)=>document.querySelector(s), out=q('#result'); let csrf='';
  ...
  async function call(params, method='GET'){
    ...
    if(method==='POST'){
      if(!csrf) throw new Error('CSRF token unavailable; refresh status');
  ...
  async function status(){
    try{
      const j=await call({action:'status'}); if(j.error)throw new Error(j.error); csrf=j.csrf||'';
  ...
  async function action(name,p1='',p2=''){
    ...
    try{const j=await call({action:name,p1,p2},'POST');out.textContent=JSON.stringify(j,null,2);setTimeout(status,500);}catch(e){out.textContent='Ошибка: '+e.message;}
  }
  document.addEventListener('click',e=>{const t=e.target.closest('button');if(!t)return;
    ...
    if(t.dataset.action)action(t.dataset.action); if(t.dataset.module)action('module',t.dataset.module,t.dataset.op||'status');
  });
