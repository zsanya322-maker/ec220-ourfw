...
mkdir -p "$R/bin" "$R/usr/bin" "$R/usr/sbin" "$R/usr/libexec" "$R/lib" "$R/usr/share/ourfw" "$R/www/ourfw/assets" "$R/lib/modules/3.4/mock" "$TMP/defaults/runtime" "$TMP/defaults/modules/smart-routing" "$TMP/defaults/modules/vpn" "$TMP/defaults/modules/nfqws" "$TMP/defaults/modules/adblock" "$TMP/defaults/modules/zram" "$TMP/defaults/modules/watchdog" "$TMP/defaults/www/assets"
...
for f in wg awg openvpn; do printf x > "$R/usr/sbin/$f"; chmod 755 "$R/usr/sbin/$f"; done
for f in nfqws zapret.sh curl openssl https-cert.sh; do printf x > "$R/usr/bin/$f"; chmod 755 "$R/usr/bin/$f"; done
printf x > "$R/usr/libexec/sftp-server"; chmod 755 "$R/usr/libexec/sftp-server"
printf x > "$R/lib/libssl.so.3"; printf x > "$R/lib/libcrypto.so.3"
