...
mkdir -p "$R/usr/bin" "$R/usr/sbin" "$R/usr/share/ourfw" "$R/www/ourfw/assets" "$R/lib/modules/3.4/mock" "$TMP/defaults/runtime" "$TMP/defaults/modules/smart-routing" "$TMP/defaults/modules/vpn" "$TMP/defaults/modules/nfqws" "$TMP/defaults/www/assets"
...
for f in nfqws zapret.sh busybox; do printf x > "$R/usr/bin/$f"; chmod 755 "$R/usr/bin/$f"; done
ln -s /usr/bin/busybox "$R/usr/bin/sha256sum"
ln -s /usr/bin/busybox "$R/usr/bin/base64"
